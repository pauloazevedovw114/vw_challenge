resource "aws_security_group" "lambda_to_rds_sg" {
  name        = "lambda_to_rds_sg"
  description = "Security group of Lambda to update events to RDS"
  vpc_id      = var.vpc_id
...
resource "aws_iam_role" "lambda_to_rds_exec" {
  name = "lambda_to_rds_exec_role"
...
resource "aws_iam_role_policy" "lambda_to_rds_vpc_access" {
  name = "lambda_to_rdsVPCAccess"
  role = aws_iam_role.lambda_to_rds_exec.id
...
data "archive_file" "lambda_to_rds_package" {
  type        = "zip"
...
resource "aws_lambda_function" "lambda_to_rds" {

  #layers = [
  #  "arn:aws:lambda:eu-central-1:934744453463:layer:psycopg2-py311:1"
  #]  
  function_name = "lambda_to_rds"
  role          = aws_iam_role.lambda_to_rds_exec.arn
  handler       = "handler.handler"
  runtime       = "python3.11"
  filename      = data.archive_file.lambda_to_rds_package.output_path
  timeout = 10
  ...
  vpc_config {
    ...
    security_group_ids = [var.lambda_to_rds_sg_id]
  }
